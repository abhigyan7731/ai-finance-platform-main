...
export async function scanReceipt(file) {
  try {
    ...
    // Log raw + cleaned response for debugging (do not leak to clients)
    console.debug("AI raw response:", text);
    console.debug("AI cleaned response:", cleanedText);

    // Try to extract the first JSON object from the response in case the
    // model wrapped it in markdown or surrounding text.
    ...
    try {
      ...
      if (!data || typeof data.amount === "undefined" || !data.date) {
        console.error("Parsed JSON missing required fields:", data, "raw:", cleanedText);
        // Treat as a non-scannable receipt instead of throwing to avoid 500s
        return null;
      }
      ...
      // Don't throw here — return null so callers can continue gracefully.
      return null;
    }
  } catch (error) {
